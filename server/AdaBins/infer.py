...
class InferenceHelper:
    def __init__(self, pretrained_path, device='cuda:0'):
        ...
        self.predict_wrapper = PredictWrapper(self.model, self.min_depth, self.max_depth)
    ...
    def predict(self, image):
        ...
        return self.predict_wrapper(image)
...
class PredictWrapper(nn.Module):
    ...
    def __init__(self, model, min_depth, max_depth):
        super().__init__()
        self.model = model
        # self.model = self.model.half()
    ...
    def forward(self, image):
        ...
        # # Flip
        # image_flipped = image.flip([-1])
        # pred_lr = self.model(image_flipped)[-1]
        # pred_lr = torch.clamp(pred_lr.flip([-1]), self.min_depth, self.max_depth)
        ...
        return centers, final.detach().cpu().numpy()
